...
export default function AdminPage() {
  ...
  return (
    // ★ 全体の最大幅を max-w-[1500px] に制限し、左右に適度な余白とゆとりを持たせました
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          {[
            ...
            <div key={idx} className="bg-slate-50 p-6 rounded-2xl border border-slate-200/80 space-y-4 flex flex-col justify-between">
              <div className="space-y-3">
                <h3 className="font-bold text-base text-orange-600 uppercase tracking-wider">{sec.title}</h3>
                
                {/* 各追加フォーム */}
                {sec.isSub ? (
                  <div className="space-y-3">
                    <input type="text" placeholder="外注会社名" value={form.subComp || ''} className="w-full p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, subComp: e.target.value})} />
                    <div className="grid grid-cols-12 gap-2">
                      <input type="text" placeholder="作業内容" value={form.subTask || ''} className="col-span-7 p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, subTask: e.target.value})} />
                      <input type="number" placeholder="単価" value={form.subPrice || ''} className="col-span-5 p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, subPrice: e.target.value})} />
                    </div>
                    <button onClick={() => addMaster('subcontractors', {company: form.subComp, task: form.subTask, price: Number(form.subPrice)||0}, ['subComp', 'subTask', 'subPrice'])} className="w-full bg-orange-600 hover:bg-orange-700 text-white py-3 rounded-xl font-bold text-base shadow-sm transition">追加</button>
                  </div>
                ) : sec.isDisp || sec.isScrap ? (
                  <div className="space-y-3">
                    <input type="text" placeholder={sec.isDisp ? "処分場名" : "スクラップ場名"} value={form[sec.isDisp ? 'dLoc' : 'sLoc'] || ''} className="w-full p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, [sec.isDisp ? 'dLoc' : 'sLoc']: e.target.value})} />
                    <div className="grid grid-cols-12 gap-2">
                      <input type="text" placeholder="品目" value={form[sec.isDisp ? 'dItem' : 'sItem'] || ''} className="col-span-4 p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, [sec.isDisp ? 'dItem' : 'sItem']: e.target.value})} />
                      <input type="text" placeholder="単位" value={form[sec.isDisp ? 'dUnit' : 'sUnit'] || ''} className="col-span-3 p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, [sec.isDisp ? 'dUnit' : 'sUnit']: e.target.value})} />
                      <input type="number" placeholder="単価" value={form[sec.isDisp ? 'dPrice' : 'sPrice'] || ''} className="col-span-5 p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, [sec.isDisp ? 'dPrice' : 'sPrice']: e.target.value})} />
                    </div>
                    <button onClick={() => addMaster(sec.key, {location: form[sec.isDisp ? 'dLoc' : 'sLoc'], item: form[sec.isDisp ? 'dItem' : 'sItem'], unit: form[sec.isDisp ? 'dUnit' : 'sUnit'] || 't', price: Number(form[sec.isDisp ? 'dPrice' : 'sPrice'])||0}, sec.isDisp ? ['dLoc', 'dItem', 'dUnit', 'dPrice'] : ['sLoc', 'sItem', 'sUnit', 'sPrice'])} className="w-full bg-orange-600 hover:bg-orange-700 text-white py-3 rounded-xl font-bold text-base shadow-sm transition">追加</button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <input type="text" placeholder={sec.placeholders[0]} value={form[sec.addForm[0]] || ''} className="w-full p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, [sec.addForm[0]]: e.target.value})} />
                    <input type="number" placeholder={sec.placeholders[1]} value={form[sec.addForm[1]] || ''} className="w-full p-3.5 border border-slate-300 rounded-xl text-base bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20" onChange={e=>setForm({...form, [sec.addForm[1]]: e.target.value})} />
                    <button onClick={() => addMaster(sec.key, {name: form[sec.addForm[0]], price: Number(form[sec.addForm[1]])||0}, sec.addForm)} className="w-full bg-orange-600 hover:bg-orange-700 text-white py-3 rounded-xl font-bold text-base shadow-sm transition">追加</button>
                  </div>
                )}
              </div>

              {/* 一覧リスト（並び替えボタン付き） */}
              <div className="max-h-60 overflow-y-auto divide-y divide-slate-100 bg-white border border-slate-200 rounded-2xl p-3 space-y-2.5 mt-4">
                {(settings[sec.key] || []).length === 0 ? (
                  <p className="text-base text-slate-400 text-center py-4">登録データがありません</p>
                ) : (
                  (settings[sec.key] || []).map((item:any, idx:number)=>(
                    <div key={idx} className="py-2.5 flex justify-between items-center text-base font-medium gap-2">
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button 
                          type="button" 
                          onClick={() => moveMasterItem(sec.key, idx, 'up')} 
                          disabled={idx === 0}
                          className="w-7 h-7 bg-slate-100 hover:bg-slate-200 disabled:opacity-30 rounded-lg text-sm font-bold flex items-center justify-center transition"
                          title="上へ"
                        >
                          ▲
                        </button>
                      ...
                      <span className="truncate text-slate-900 font-bold flex-1 px-1 text-base">
                        {sec.isSub ? `${item.company} / ${item.task}` : sec.isDisp || sec.isScrap ? `${item.location} (${item.item}/${item.unit})` : item.name}
                      </span>
                      
                      <div className="flex items-center gap-1.5 shrink-0">
                        <span className="text-slate-400 text-sm">¥</span>
                        <input type="number" value={item.price || 0} onChange={(e)=>updateItemPrice(sec.key, idx, 'price', e.target.value)} className="w-24 p-2.5 border border-slate-300 rounded-xl text-right text-base font-bold bg-slate-50" />
                        <button type="button" onClick={()=>deleteMaster(sec.key, idx)} className="text-rose-500 hover:text-rose-700 font-bold text-sm ml-1.5 p-1">削除</button>
                      </div>
                    </div>
                  ))
                )}
